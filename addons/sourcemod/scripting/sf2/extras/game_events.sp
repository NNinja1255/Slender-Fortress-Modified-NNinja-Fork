...
Action Event_PlayerDeath(Event event, const char[] name, bool dB)
{
	...
	if (!fake)
	{
		...
		else
		{
			...
			if ((g_EscapeEliminationConVar.BoolValue && !SF_SpecialRound(SPECIALROUND_1UP) && !(IsRoundInEscapeObjective() || IsRoundEnding() || g_RoundTime <= 0) && !SF_IsRenevantMap() && !SF_IsSlaughterRunMap() && !SF_IsBoxingMap()) && !DidClientEscape(client) && GetClientTeam(client) == TFTeam_Red)
			{
				g_PlayerEliminated[client] = false;
			}
		}
